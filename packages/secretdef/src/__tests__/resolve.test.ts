...
describe('resolveSecret', () => {
  it('resolves a secret from process.env', () => {
    const info = resolveSecret('STRIPE_KEY', base, 'production', {
  ...
  it('applies envOverrides for matching environment', () => {
    ...
    const info = resolveSecret('STRIPE_KEY', spec, 'dev', {
  ...
  it('uses default value from envOverrides', () => {
    const spec: SecretSpec = {
      ...base,
      envOverrides: {
  ...
  it('falls back to base spec when no override matches', () => {
    const spec: SecretSpec = {
      ...base,
      envOverrides: {
        dev: { envVar: 'STRIPE_TEST_SECRET_KEY' },
    ...
    const info = resolveSecret('STRIPE_KEY', spec, 'production', {
